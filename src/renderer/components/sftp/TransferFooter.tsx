...
export const TRANSFER_FOOTER_HEIGHT = 56;
...
const TransferFooter: React.FC<TransferFooterProps> = ({ pane, transfer, formatSize, formatDuration }) => {
  ...
  const progressPercent = transfer.progress;
  const sizeLabel = transfer.totalBytes ? formatSize(transfer.totalBytes) : null;
  const transferredLabel = transfer.bytesTransferred ? formatSize(transfer.bytesTransferred) : null;
  ...
  const multiFileLabel = transfer.totalFiles && transfer.totalFiles > 1
